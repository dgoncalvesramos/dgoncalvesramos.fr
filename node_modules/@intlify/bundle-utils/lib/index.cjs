...
const {
  ...
  detectHtmlTag,
  LocationStub
} = require$1("@intlify/message-compiler");
function createCodeGenerator(options = {
  ...
  const { sourceMap: sourceMap$1, source, filename } = options;
...
function mapLinesColumns(resMap, codeMaps, inSourceMap) {
  ...
  const resMapConsumer = new sourceMap.SourceMapConsumer(resMap);
  const inMapConsumer = inSourceMap ? new sourceMap.SourceMapConsumer(inSourceMap) : null;
  ...
  resMapConsumer.eachMapping((res) => {
    ...
    const mapConsumer = new sourceMap.SourceMapConsumer(map);
...
function generate$2(targetSource, {
  ...
  const ast = jsoncEslintParser.parseJSON(value, { filePath: filename });
  if (legacy && type === "sfc") {
      ...
      switch (node2.type) {
        case "Program":
          if (type === "plain") {
            ...
            generator.indent();
            const componentVariable = bridge ? `Component.options || Component` : useClassComponent ? `Component.__o || Component` : `Component`;
...
function generate$1(targetSource, {
  ...
  const generator = createCodeGenerator(options);
  const ast = yamlEslintParser.parseYAML(value, { filePath: filename });
  if (legacy && type === "sfc") {
...
const _require = module$1.createRequire((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (document.currentScript && document.currentScript.src || new URL('index.cjs', document.baseURI).href)));
